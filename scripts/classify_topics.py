...
【关于"评估与基准"的判断规则——非常重要，先看这里】
"评估与基准" 仅适用于"**核心贡献是产出新 benchmark / 评测数据集 / 评测方法本身**"的论文，不要把"包含评估实验的训练/Agent/RAG 论文"误分进来。

**强信号（命中即倾向 评估与基准）**：
- 项目英文名以 Bench/Benchmark/Eval/Arena/Leaderboard 结尾或为独立词（例如 ADBench、ClawEval、AgentSearchBench、HybridRAG-Bench）
- 标题/摘要明确说"我们提出/构建了一个 XX 基准/评测/数据集来评估……"

**反信号（即使含评估字样也不要分到评估与基准）**：
- 论文核心是训练方法/RL/数据合成/Agent 框架，只是顺带做了 benchmark 实验 → 按其核心贡献分类
  - 例：CoVe（数据合成训练工具调用 Agent）→ Agent 智能体
  - 例：Composer2（编程 Agent 模型训练）→ 工程实践与 Coding Agent
  - 例：TREX（Agent 驱动的微调自动化）→ Agent 智能体
  - 例：GraSP（图结构技能编排）→ Agent 智能体
- 名称里仅出现 Eval/Evaluation 作动词（"用 X 评估 Y"）而项目本身不是评测套件 → 不分到评估与基准
- 标题含"评估"但讲的是分析/局限性/理论 → 看它分析的是什么主题，按主题归类
...
6. "评估与基准"：benchmark、评测方法、新数据集（也参考上面的优先级规则）
7. "模型架构与训练"：新模型权重、新架构、注意力变体、PEFT、量化、预训练
8. "AI 科研"：自动科研 Agent、Auto-Research、AI Scientist、自动文献综述/论文发现类
