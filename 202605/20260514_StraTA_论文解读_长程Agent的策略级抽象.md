...
![图1：反应式 Agent 与 StraTA 框架的对比。(a) 反应式 Agent 每一步从局部状态生成动作，规划和执行同时进行，容易出现短视探索、不必要的回退和不一致的行为。(b) StraTA 先生成一条紧凑的全局策略，所有后续动作都基于该策略生成，行为更连贯一致。(c) 在长程 Agent 任务上，显式的策略指引明显改善学习效率和最终性能。](https://www.mulanai.com/fs/files/0525_78522e0b_teaser.png)
...
![图2：StraTA 框架总览。(a) 给定任务，StraTA 生成多个策略并为每个策略执行多次 rollout，构成层级化的 group 进行策略级和动作级优化。(b) 多样化策略 rollout 通过最远点采样在语义空间选取彼此差异最大的策略，提升策略空间的探索。(c) Critical self-judgment 识别那些既不遵循策略也不推动任务进展的动作步，赋予步级辅助惩罚以实现细粒度信用分配。](https://www.mulanai.com/fs/files/0525_ffc36253_method.png)
...
![图3：StraTA 各变体在 ALFWorld 上的训练曲线。Diverse 组件让早期收敛速度明显加快（红/橙超越浅黄），Judgment 组件主要在中后期把上限拉高。两者结合后曲线既快又高。](https://www.mulanai.com/fs/files/0525_dcd32ee9_ablation.png)
...
![图4：计算效率分析。(a) PPO/GRPO/StraTA 每步 wall-clock 时间对比；(b) StraTA 各阶段细分。Diverse 和 Judgment 两个组件的额外开销加起来不到 12 秒，相对于 175 秒的 training 几乎可以忽略。](https://www.mulanai.com/fs/files/0525_a5008d51_ablation.png)
